...
function buildLifestylePrompt(args) {
    const { product, brandDna, sceneOptions, useGoldenBackground, creativeFreedom, imageLayout = { hasProductRefs: true, hasBackgroundRef: false, hasModelRefs: false }, runtimeInput, globalHardRules, productHardRules } = args;
    ...
    if (brandDna?.text) {
        ...
        blocks.push("Brand: AuréLéa. Visual style: calm, minimal, feminine, premium; soft natural lighting, warm tones, elegant negative space.");
    }
    ...
    if (creativeFreedom === true && !runtimeInput) {
        blocks.push("Choose a sporty, elegant pose and styling that fits the brand; outfit and setting can vary within the brand DNA.");
    }
    else {
        const sceneLines = [
    ...
    // --- Task line ---
    blocks.push(`Generate exactly one high-quality Amazon-style lifestyle product image: a model performing an exercise or pose with the ${productName}. The product must be clearly visible and recognisable from the reference(s). The image should be suitable for e-commerce (product listing). Output only the generated image, no text.`);
 ...
 */
function buildRuntimeExtras(rt) {
